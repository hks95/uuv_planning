...
def publish_control_wrench(cur_vel_command):
    ...
    global is_started
    # pdb.set_trace()
    print("cur vel: ", cur_vel_command)
    print("prev vel: ", prev_vel_command)
    
    accel_command = (cur_vel_command - prev_vel_command)/0.1

    # print("acc : ", accel_command)
    if is_started is 0:
        accel_command=np.array([[0,0,-1,0,0,0]])
        is_started = 1
    ...
    prev_vel_command = cur_vel_command
    print("cmd_force after: ", force_msg)
...
while not rospy.is_shutdown():
    ...
    command_raw = sess.run(output, feed_dict = {states : input_vector})
    command[0,:3] = command_raw
