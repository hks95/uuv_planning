...
layer_1 = tf.layers.dense(inputs=layer_0,units = 128,activation=tf.nn.sigmoid)
layer_2 = tf.layers.dense(inputs=layer_1,units = 128,activation=tf.nn.sigmoid)
layer_3 = tf.layers.dense(inputs=layer_2,units = 128,activation=tf.nn.sigmoid)
...
def publish_control_wrench(cur_vel_command):
    ...
    force_msg.wrench.force.x = 1000*wrench[0]
    force_msg.wrench.force.y = 1000*wrench[1]
    force_msg.wrench.force.z = 1000*wrench[2]

    force_msg.wrench.torque.x = 0.0 #10*wrench[3]
    force_msg.wrench.torque.y = 0.0 #10*wrench[4]
    force_msg.wrench.torque.z = 0.0 #10*wrench[5]
...
#For counting whether the UUV has surfaced or not
prev_height =0
prev_x = -20
surface_count = 0
count_since_last_surfacing = 0    
while not rospy.is_shutdown():
    ...
    # print('raw :',command)
    #Local Minima Fix. If dept is under 2.5 meters, and cmd_vel.linear.x is les than 0.1 throttle up to break away from current
    #If distance is near 20 by error of 0.4, and we are not at dept of 2.5 meters, thrusters go down and all throttles are less then 0.1
    
    #if poseZ<2.5 and np.sum(abs(command))<0.1:
    #    command = np.array([1,0,0])
    # if np.sum(abs(command))<0.2:
    #     #Null value occured before diving
    #     # print('local minima escape')
    #     if poseZ>-1.8 :
    #         if np.sqrt(np.sum(np.array([poseX+20,poseY,poseZ])**2))<1:
    #             command = np.array([[0,0,-1]])
            
    #     #Null value occured at after diving
    #     if poseZ<-1.8:
    #         if np.sqrt(np.sum(np.array([poseX,poseY,poseZ])**2))>1:
    #             command = np.array([[1,0,0]])
            
    #     #Null value occured near goalpoint
    #         else:
    #             command = np.array([[0,0,1]])
        #Null 
