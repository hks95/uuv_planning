...
def publish_control_wrench(cur_vel_command):
    ...
    force_msg.wrench.force.x = 10*wrench[0]
    force_msg.wrench.force.y = 10*wrench[1]
...
while not rospy.is_shutdown():
    ...
    if np.sum(abs(command)<0.2):
        ...
        if poseZ>-2.5 and np.sqrt(np.sum(np.array([poseX+20,poseY,poseZ])**2))<1:
            command = np.array([[0,0,-1]])
            
        #Null value occured at after diving
        if poseZ<-2.5 and np.sqrt(np.sum(np.array([poseX,poseY,poseZ])**2))>1:
            command = np.array([[1,0,0]])
